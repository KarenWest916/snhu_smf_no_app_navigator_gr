...
smf_no_app_df <- dbGetQuery(
  ...
  statement = read_file(
    file = here::here("sql","smf_no_app_gr_results.sql")
  )
) %>% as_tibble()
```
...
smf_no_app_results <- 
 ...
 gt() %>%
   ...
   tab_header(
    title = md("SMF No App Navigator (GR): Test vs. Control")) %>% 
    tab_source_note(
    source_note = c("Data Source:  Unify Opportunity - retrieved on 2/24/21"))
  #%>%
...
smf_no_app_results %>% gtsave(filename = "smf_no_app_nav_gr_20210224.png", path = here::here("imgs"))
```